...
class SSSPGraphUI:
    ...
    def delete_edge(self, u, v):
        start = time.perf_counter()
        if v in self.all_users[u].out_edges:
            del self.all_users[u].out_edges[v]
        if u in self.all_users[v].in_edges:
            del self.all_users[v].in_edges[u]
        user_v = self.all_users[v]
    ...
    def delete_node(self, target_id):
        if target_id not in self.all_users or target_id == self.source_user_id:
            return -1.0
        start = time.perf_counter()
        ...
        for neighbor_id in list(target_user.in_edges.keys()):
            if neighbor_id in self.all_users:
                ...
                if target_id in neighbor.out_edges:
                    del neighbor.out_edges[target_id]
        for neighbor_id in list(target_user.out_edges.keys()):
            if neighbor_id in self.all_users:
                neighbor = self.all_users[neighbor_id]
                if target_id in neighbor.in_edges:
                    del neighbor.in_edges[target_id]
    ...
    def simulate_batch_update(self, batch_df):
        ...
        if has_node_ops:
            ...
            for msg in initial_delete_messages:
                self.message_queue.append(msg)
            for msg in initial_add_messages:
                self.message_queue.append(msg)
            self.process_queue()
            self._trigger_recomputation()
    ...
    def get_recommendations(self, top_n=5):
        ...
        source = self.all_users[self.source_user_id]
        direct_neighbors = set(source.out_edges.keys())

        candidates = []
        for uid, user in self.all_users.items():
            if uid == self.source_user_id:
                continue
            if uid in direct_neighbors:
                continue
            ...
            strength = round(1.0 / user.distance, 2) if user.distance > 0 else 999.0

            candidates.append({
                "id": uid,
                "username": user.username,
                "distance": round(user.distance, 4),
                "hops": hops,
                "strength": strength,
            })
